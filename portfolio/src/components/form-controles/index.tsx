...
interface FormControlesProps {
  ...
  getInputClassName?: (controle: Controle) => string;
}
      ...
      {controles.map((controleItem, index) => (
        <div key={index} className="mb-4">
          ...
          <input
            ...
            className={
              getInputClassName
                ? getInputClassName(controleItem)
                : "border shadow rounded w-full py-2 px-3 text-gray-700 tracking-wide focus:outline-none focus:shadow-outline"
            }
          />
        </div>
      ))}
